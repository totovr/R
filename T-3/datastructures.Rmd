...
Also with the position, it needs to have double bracket 
```{r}
L[[2]]
```
...
**sapply** casts the data frame to a list before operating on it, **apply** casts the data frame to a matrix. Since matrices cannot have mixed types and there is at least one column with non-numeric data, then everything becomes character data.
